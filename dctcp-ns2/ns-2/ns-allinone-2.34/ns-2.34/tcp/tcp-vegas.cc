...
VegasTcpAgent::recv(Packet *pkt, Handler *)
{
	...
	if (tcph->seqno() > last_ack_) {
		...
		double cong_signal = rtt; //default congestion control algo is Timely				
		if(timely_==1 || hope_!=0){
			...
			if (hope_==1){
				...
				cong_signal = max_delay * 8.0 / line_rate;
			} else if (hope_==2) {
				...
				cong_signal = tot_delay * 8.0 / line_rate;
			}

			if(timely_prevRTT_ == 0) {
		...
		//printf("****\n*** rtt= %f | cong_sgnl= %f | old_cwnd= %f | old_rate= %f\n",rtt,cong_signal,(double)cwnd_,timely_rate_);
		//printf("**cur_time= %f | last_update_time= %f | base_rtt= %f\n",currentTime*1000,timely_lastUpdateTime*1000,v_baseRTT_);
			...
			if (rtt < timely_t_low_) { //additivive increase if rtt < t_low
    			...
    			} else if (normalized_gradient <= 0) { //additive increase if avg gradient <= 0 
		...
		//printf("*rtt_diff= %f | avgRTTDiff= %f | gradient= %f ",rtt_diff,timely_avgRTTDiff_,normalized_gradient);
		//printf("| delta_factor= %f | new_rate= %f\n",delta_factor,timely_rate_);
		...
		Tcl::instance().evalf("%s recv %f %f %d", this->name(), rtt, cong_signal, iph->HOPE_hop_cnt() );
